...
interface Message {
	...
	/**
	 * @param keyTerminal terminal that has been turned on
	 * @param keyCaller   terminal that tried to communicate before
	 * @return string with informative message.
	 */
	static String isAvailable(int keyTerminal, int keyCaller) {
		return "O terminal '" + keyTerminal + "' já está disponível para o terminal '" + keyCaller + "'.";
	}
	 ...
	 * @return string with informative message.
	 */
	static String isAvailableForTextCommunications(int keyTerminal, int keyCaller) {
		return "O terminal '" + keyTerminal + "' já está disponível para receber comunicações de texto de '" + keyCaller
				+ "'.";
	}
